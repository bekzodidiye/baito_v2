    ...
    <div className="w-full shrink-0 bg-white pt-4 pb-3 px-6 z-40 border-b border-slate-100 shadow-[0_2px_10px_rgba(0,0,0,0.02)]">
      <div className="relative flex items-start justify-between w-full max-w-[280px] mx-auto">
        ...
        >
          ...
          <span className={`text-[9px] font-black tracking-wider mt-2 uppercase whitespace-nowrap transition-colors ${mode === 'profile-info' ? 'text-brand-primary' : 'text-emerald-600'}`}>{t.step1}</span>
        </button>
        ...
        >
          ...
          <span className={`text-[9px] font-black tracking-wider mt-2 uppercase whitespace-nowrap transition-colors ${mode === 'documents' ? 'text-brand-primary' : mode === 'finish' ? 'text-emerald-600' : 'text-slate-400'}`}>{t.step2}</span>
        </button>
